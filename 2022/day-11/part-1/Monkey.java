...
public class Monkey {
    ...
    public void createExamineFunction(String examineOperation) {
        ...
        examineItem = (item) -> {
            ...
            long reducedWorry = Math.floorDiv(responseVal, 3);
            item.setWorryLevel(reducedWorry);
            numItemsInspected++;
            System.out.println(String.format("- After examining and forgetting, worry level for item is now %d", item.getWorryLevel()));
            return null;
        };
    }
